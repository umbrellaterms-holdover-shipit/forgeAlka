...
### Preflight pricing fallback

The chat preflight check now uses the same model/rate catalog strategy as the model dropdown:

1. use the configured local snapshot when it exists and contains the selected model;
2. otherwise fall back to the bundled starter catalog;
