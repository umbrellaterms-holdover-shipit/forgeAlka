...
def rate_card_from_payload(data: Any) -> dict[str, dict[str, Decimal]]:
    """Normalize an OpenRouter model snapshot or compact rate payload.
...
def load_rate_card(path: str | Path) -> dict[str, dict[str, Decimal]]:
    ...
    return rate_card_from_payload(raw)


def load_rate_card_with_seed_fallback(path: str | Path | None = None) -> tuple[dict[str, dict[str, Decimal]], str]:
    """Load a local rate card, falling back to the bundled starter model catalog.

    This is intentionally separate from `load_rate_card`: cost reports should be
    explicit about their input file, while chat preflight needs first-run UX that
    matches the model dropdown even before a user has written a rates file.
    """
    if path:
        p = Path(path)
        if p.exists():
            return load_rate_card(p), str(p)
    from .model_catalog import starter_models_snapshot

    return rate_card_from_payload(starter_models_snapshot()), "bundled-starter-catalog"


def rate_card_for_model_with_seed_fallback(path: str | Path | None, model: str) -> tuple[dict[str, Decimal], str, bool]:
    """Return pricing for `model`, trying the local file before the starter seed.

    Returns `(pricing, source, used_seed_fallback)`. If the local snapshot exists
    but lacks the selected model, the bundled catalog is tried as a secondary
    fallback so the dropdown and preflight modal do not disagree.
    """
    local_source = None
    if path:
        p = Path(path)
        if p.exists():
            local_rates = load_rate_card(p)
            local_source = str(p)
            if model in local_rates:
                return local_rates[model], local_source, False

    from .model_catalog import starter_models_snapshot

    seed_rates = rate_card_from_payload(starter_models_snapshot())
    if model in seed_rates:
        return seed_rates[model], "bundled-starter-catalog", True
    return {}, local_source or "bundled-starter-catalog", local_source is None
