...
from .costs import format_money, load_rate_card
...
def cost_confirmation_check(ctx: PreflightContext) -> PreflightCheck:
    ...
    if ctx.rates_path and ctx.rates_path.exists():
        rates = load_rate_card(ctx.rates_path)
        pricing = rates.get(ctx.model, {})
